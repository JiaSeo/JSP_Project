...
<script type="text/javascript">
	...
	// 지역 선택 1
	function getSLoc(upper_locno) { // this.value = upper_locno
		var s_loc100 = document.getElementById("s_loc100"); // id가 s_loc100인 태그를 가져온다. s_loc100 : 서울시 하위 지역
		var s_loc200 = document.getElementById("s_loc200"); // s_loc200 : 경기도 하위 지역
		
		if(upper_locno == "100") { // 대분류가 100이면
			s_loc100.style.display = "inline"; // 100의 소분류를 보여준다
		} else {
	...
	function getSSubject(b_subjectno){
		var s_subj400 = document.getElementById("s_subject400"); // id가 s_subject400인 태그를 가져온다. 사회탐구를 선택하면 하위 해당 하위 과목을 보여주는 기능
								...
								<table class="table">
									<tbody class="table-border-bottom-0">
										...
										<tr>
											<td>과목</td>
											<td>
												<select id="b_subject" name="b_subject" onchange="getSSubject(this.value)"> <%-- this.value 는 select에서 선택한 값 ${b_subj.subjectno }이 들어감 --%>
										...
										<tr>
											...
											<td>
												<div>
													<select id="b_loc" name="b_loc" onchange="getSLoc(this.value)"> <%-- this.value 는 select에서 선택한 값 ${b_loc.locno }이 들어감 --%>
